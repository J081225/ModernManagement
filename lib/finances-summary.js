...
async function computeFinancesSummary({ db, workspace, period, start, end, env, now }) {
  ...
  // money_out: the BG2 expenses table does not exist yet — the feed is
  // wired at 0 here ("expenses table pending BG2") so the shape is
  // stable; legacy PM expenses read through until it supersedes them.
  const expensesCents = 0; // expenses table pending BG2
  ...
  try {
    ...
    if (aR.rows[0]) {
      ...
      // (BG2: the expenses table joins the out-since sum here.)
    }
  } catch (err) {
  ...
  return {
    ...
    anchor,
    goal,
    by_category: legacyOut.by_category,
  };
}
