...
async function composeLedgerRows({ db, workspace, period, start, end, env, direction, category, source }) {
  const p = resolvePeriod({ workspace, period, start, end });
  const live = depositsLive(env);
  const wsId = workspace.id;
  const owner = workspace.owner_user_id;
  const tz = wsTz(workspace);
  ...
  if (wantSource('ledger')) {
    const r = await db.query(
      `SELECT tp.amount_cents, tp.payment_type, tp.payment_method, tp.created_at,
  ...
  if (wantSource('expenses')) {
    const r = await db.query(
      `SELECT amount_cents, category, description, vendor, spent_on, source AS row_source
         FROM expenses
        WHERE workspace_id = $1
          AND spent_on >= $2::date AND spent_on < $3::date
