...
function buildSystemPrompt({ workspace, contact, knowledge, callerAppointments, menu, thread, channel }) {
  ...
  lines.push('- BOOKING: they want an appointment -> confirm service and time (check propose_appointment_times before agreeing to ANY time), get a name if unknown -> book_appointment. Exit: booked or sent for confirmation, stated plainly.');
  ...
  lines.push('- DAY-OF LOGISTICS: "running late" / "on my way" about today -> acknowledge warmly and make sure the team sees it (add_task with the message and time). Exit: "I\'ve let them know - see you soon."');
  lines.push('- COMPLAINT: unhappy about service, staff, or an experience -> empathize and take it seriously. NEVER argue. NEVER promise refunds, discounts, or compensation - you do not have that authority. Say honestly: "I\'ll make sure the owner sees this today", then escalate_appointment_to_owner with the reason. Exit: escalated; do not re-litigate.');
  lines.push('- UNKNOWN QUESTION: not answerable from the knowledge above -> say so honestly and promise a follow-up ("I\'ll find out and someone will get back to you"), then escalate_appointment_to_owner. NEVER bluff or invent an answer. Exit: follow-up promised and escalated.');
...
module.exports = { processInboundMessage, buildSystemPrompt };
