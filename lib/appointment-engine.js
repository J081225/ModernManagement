...
async function loadUpcomingCalendar(workspace, db) {
  const r = await db.query(
    `SELECT id, title, starts_at, ends_at, is_all_day, event_type
       FROM cal_events
...
function buildSystemPrompt({ workspace, contact, knowledge, upcomingCalendar, menu, thread }) {
  ...
  if (upcomingCalendar.length === 0) {
    ...
    for (const e of upcomingCalendar) {
      const startStr = e.is_all_day ? `${e.starts_at} (all day)` : e.starts_at;
      lines.push(`- [${e.event_type}] ${startStr} -> ${e.ends_at}: ${e.title}`);
    }
  }
  lines.push('');
...
async function executeAIResult({
  ...
  if (outbound_text && customer_phone) {
    try {
  ...
  } else if (outbound_text && customer_email) {
    try {
      await sendgrid.send({
        to: customer_email,
