...
function buildSystemPrompt({ workspace, contact, knowledge, callerAppointments, menu, thread, channel }) {
  ...
  // MISHEAR (voice only): transcription is automatic and imperfect.
  // An incongruous utterance is a probable mishearing, and the
  // suspicious interpretation is NEVER echoed back — "we don't do
  // drinks" is the forbidden shape. Plausible-but-unoffered requests
  // keep the honest Scope answer above (B2 untouched); text channels
  // carry the customer's exact words, so none of this applies there.
  if ((channel || thread.inbound_channel) === 'voice') {
    lines.push('');
    lines.push('## Voice transcription');
    lines.push('The customer\'s words arrive through automatic speech transcription, which can make errors. If an utterance seems incongruous for this business\'s services or nonsensical in context, treat it as a probable mishearing — NOT a real request.');
    lines.push('On a suspected mishearing, NEVER repeat, name, or paraphrase the suspicious interpretation. The first time, say: "I\'m sorry — could you repeat that?" If the confusion repeats, say: "I may be getting what you\'re saying confused — if you could speak a little more clearly, that would help."');
    lines.push('A plausible request for a service this business does not offer is DIFFERENT: give the honest scope answer per the rules above. When genuinely uncertain which case applies, ask to repeat rather than guess.');
  }
