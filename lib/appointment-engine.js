...
async function findOrCreateThread({
  ...
  const inserted = await db.query(
    `INSERT INTO appointment_threads
       (workspace_id, contact_id, inbound_channel, customer_phone, customer_email, state, ai_paused, paused_at, paused_by)
     VALUES ($1, $2, $3, $4, $5, 'gathering', $6, $7, $8) RETURNING *`,
...
function buildSystemPrompt({ workspace, contact, knowledge, callerAppointments, menu, thread, channel }) {
  ...
  lines.push('');
  lines.push('## Language');
  if (workspace.customer_language === 'es') {
    lines.push('Greet and reply in Spanish by default. If the customer writes in a different language, follow the customer and reply in their language.');
  } else {
    lines.push('Greet and reply in English by default. If the customer writes in a different language, follow the customer and reply in their language.');
  }
