...
async function processInboundMessage({
  ...
  // SPEECH-GRADE: relay hook — speaks the model's pre-tool text as a
  // latency bridge while tools run (voice only; null elsewhere).
  onSpeechSegment = null,
  ...
  const result = await executeAIResult({
    ...
    onLanguageSwitch,
    // SPEECH-GRADE: the compose pass needs the same system + tools AND
    // the per-call client (the 100%-fallback outage: a module-scope
    // client was assumed; none exists); the relay's onSpeechSegment
    // (voice only) speaks the pre-tool bridge.
    systemPrompt, tools, onSpeechSegment, anthropicClient: anthropic,
  });
...
function buildSystemPrompt({ workspace, contact, knowledge, callerAppointments, menu, thread, channel }) {
  ...
  lines.push('- If a requested day is closed, say so plainly ("We\'re closed Mondays") and offer the nearest open day — a closed day is never a mystery gap or a silent skip.');
  ...
  if ((channel || thread.inbound_channel) === 'voice') {
    ...
    lines.push('If the caller was greeted by name but indicates they are someone else ("this is his wife", "no, this is her phone"), adapt naturally — a light acknowledgment, ask who you\'re speaking with if it matters, and NEVER reuse the wrong name for the rest of the call.');
    // DIRECT-ANSWER: lead with the answer. The R3 verification
    // sentence remains the ONLY confirmation moment, at booking time.
    lines.push('');
    lines.push('## Answer first');
    lines.push('When a caller asks for availability, LEAD with the slots — no restating their request, no "you\'d like...", no narrated verification before the answer. Answer first, details after, and never re-say what the caller just said.');
    lines.push('FORBIDDEN example — "You\'re looking for something between the 1st and the 3rd? Let me check... you wanted before 3 PM, correct? We have..." CORRECT — "Sure — on the 1st we have 9:00 and 11:30, the 2nd is open at 2:00, and the 3rd has 5:30."');
    lines.push('The single "I\'m verifying you want..." sentence at booking time is the ONLY moment you restate anything.');
  }
...
// SPEECH-GRADE: the post-tool compose pass. One extra model call turns
// tool RESULTS (data) into dialogue. Any tool_use in ITS output is
// ignored — text blocks only, no loops.
//
// URGENT fix (the 100%-fallback outage): the Anthropic client is
// per-call inside processInboundMessage, NOT module scope — the first
// version referenced a module-level `anthropic` that does not exist
// (instant ReferenceError, every call). The client is now an EXPLICIT
// required parameter, and the payload is built by a PURE exported
// function so the gate can validate the real shape with a mock
// transport.
function _buildComposePayload({ systemPrompt, tools, body, aiResponse, toolResults }) {
  const directive = '\n\n## Speak the result\nThe tool results in this turn are DATA, not dialogue. Reply with ONE short, natural spoken sentence (two at most) telling the caller the outcome in plain speech — no labels, no ISO dates, no parentheses, never "Booked:" and never "Calendar updated". If a tool result is already a first-person sentence addressed to the caller (like a booking-approval line), say that sentence VERBATIM. Do not call tools.';
  return {
    model: ANTHROPIC_MODEL,
    max_tokens: 300,
    system: (systemPrompt || '') + directive,
    tools: tools || [],
    messages: [
      { role: 'user', content: body },
      { role: 'assistant', content: aiResponse.content },
      { role: 'user', content: toolResults.map((t) => ({ type: 'tool_result', tool_use_id: t.tool_use_id, content: t.content })) },
    ],
  };
}

async function _composeSpokenResult({ anthropicClient, systemPrompt, tools, body, aiResponse, toolResults }) {
  if (!anthropicClient) throw new Error('anthropicClient required — compose pass cannot run without the per-call client');
  const followUp = await anthropicClient.messages.create(
    _buildComposePayload({ systemPrompt, tools, body, aiResponse, toolResults })
  );
...
async function executeAIResult({
  ...
  customer_phone, customer_email, db, twilio, sendgrid, env, logger,
  onLanguageSwitch = null,
  // SPEECH-GRADE: voice replies are MODEL SPEECH ONLY. systemPrompt +
  // tools feed the post-tool compose pass; onSpeechSegment lets the
  // relay speak the model's pre-tool text as a natural bridge while
  // tools run; composeSpeech is the injectable pass (tests).
  systemPrompt = null, tools = [], onSpeechSegment = null, composeSpeech = null,
  anthropicClient = null,
}) {
  const used_tools = [];
  let outbound_text = '';
  // SPEECH-GRADE: on voice, tool messages are DATA, not dialogue —
  // collected here for the compose pass, never appended to the spoken
  // reply. Text channels keep the original concatenation.
  const speechMode = channel === 'voice';
  const toolResults = [];
  ...
  for (const block of (aiResponse.content || [])) {
    if (block.type === 'text') {
      ...
      if (decision === 'decline') {
        ...
        toolResults.push({ tool_use_id: block.id, content: declineMsg });
        if (!speechMode && !outbound_text.includes(declineMsg)) {
          outbound_text += (outbound_text ? '\n' : '') + declineMsg;
        }
        continue;
      }
      if (decision === 'queue') {
        ...
        toolResults.push({ tool_use_id: block.id, content: queuedMsg });
        if (!speechMode && !outbound_text.includes(queuedMsg)) {
          outbound_text += (outbound_text ? '\n' : '') + queuedMsg;
        }
        continue;
      }
      ...
      try {
        ...
        used_tools.push({ name: tool.name, success: !!r.success, message: r.message, data: r.data, input: block.input });
        toolResults.push({ tool_use_id: block.id, content: r.message || (r.success ? 'Done.' : 'The tool did not succeed.') });
        if (!speechMode && r.message && !outbound_text.includes(r.message)) {
          outbound_text += (outbound_text ? '\n' : '') + r.message;
        }
      } catch (err) {
        ...
        toolResults.push({ tool_use_id: block.id, content: 'The tool failed: ' + String(err.message).slice(0, 200) });
      }
    }
  }

  // SPEECH-GRADE: the post-tool compose pass. The model turns the tool
  // RESULTS into ONE natural spoken sentence; the raw tool messages
  // never reach TTS. The pre-tool text becomes the latency bridge
  // (spoken immediately via onSpeechSegment when the relay provides
  // it). Fallbacks are speech-grade by construction — never a tool
  // message.
  if (speechMode && toolResults.length) {
    const bridge = outbound_text;
    let bridged = false;
    if (bridge && typeof onSpeechSegment === 'function') {
      try { onSpeechSegment(bridge); bridged = true; } catch (err) {
        logger.error('[appointment-engine] bridge speak failed (composing anyway):', err.message);
      }
    }
    let composed = '';
    try {
      composed = await (composeSpeech
        ? composeSpeech({ body, aiResponse, toolResults })
        : _composeSpokenResult({ anthropicClient, systemPrompt, tools, body, aiResponse, toolResults }));
    } catch (err) {
      logger.error('[appointment-engine] speech compose failed (deterministic fallback):', err.message);
    }
    if (!composed || !String(composed).trim()) {
      // URGENT hardening: the fallback SPEAKS THE ANSWER when it can —
      // deterministic per-tool formatters (plain code, no model) for
      // the schedule tools; "that's all set" survives ONLY for action
      // tools with nothing to report; read tools never claim success
      // they can't show.
      const { speakToolFallback } = require('./speech-fallbacks');
      composed = speakToolFallback(used_tools, workspace, registry);
    }
    outbound_text = (bridge && !bridged) ? bridge + '\n' + composed : composed;
  }
...
module.exports = { processInboundMessage, buildSystemPrompt, executeAIResult, findOrCreateThread, appendOwnerTurnToContext, _composeSpokenResult, _buildComposePayload };
