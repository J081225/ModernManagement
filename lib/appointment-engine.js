...
async function processInboundMessage({
  ...
  const upcomingCalendar = await loadUpcomingCalendar(workspace, db).catch((err) => {
    logger.error('[appointment-engine] calendar load failed (continuing):', err.message);
    return [];
  });
  ...
  const systemPrompt = buildSystemPrompt({ workspace, contact, knowledge, upcomingCalendar, menu, thread });
...
async function loadUpcomingCalendar(workspace, db) {
  // CP5: canceled appointments keep their cal_events row for the UI but
  // are excluded here so the AI never treats a canceled slot as busy.
  const r = await db.query(
    `SELECT ce.id, ce.title, ce.starts_at, ce.ends_at, ce.is_all_day, ce.event_type
       FROM cal_events ce
       LEFT JOIN appointments a ON a.id = ce.appointment_id
      WHERE ce.workspace_id = $1
        AND ce.starts_at >= NOW()
        AND ce.starts_at < NOW() + INTERVAL '14 days'
...
function buildSystemPrompt({ workspace, contact, knowledge, upcomingCalendar, menu, thread }) {
  const lines = [];
  ...
  if (upcomingCalendar.length === 0) {
    lines.push('(No events on the calendar — the schedule is open subject to business hours from the knowledge base.)');
  } else {
    for (const e of upcomingCalendar) {
      const startStr = e.is_all_day ? `${fmtEventTime(e.starts_at)} (all day)` : fmtEventTime(e.starts_at);
      lines.push(`- [${e.event_type}] ${startStr} -> ${fmtEventTime(e.ends_at)}: ${e.title}`);
    }
  }
  lines.push('');
