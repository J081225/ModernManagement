...
async function processInboundMessage({
  ...
  const result = await executeAIResult({
    ...
    onLanguageSwitch,
    // SPEECH-GRADE: the compose pass needs the same system + tools; the
    // relay's onSpeechSegment (voice only) speaks the pre-tool bridge.
    systemPrompt, tools, onSpeechSegment,
  });
...
async function _composeSpokenResult({ systemPrompt, tools, body, aiResponse, toolResults }) {
  ...
  if (speechMode && toolResults.length) {
    ...
    try {
      composed = await (composeSpeech
        ...
        : _composeSpokenResult({ systemPrompt, tools, body, aiResponse, toolResults }));
    } catch (err) {
      logger.error('[appointment-engine] speech compose failed (fallback line):', err.message);
    }
    if (!composed || !String(composed).trim()) {
      composed = used_tools.some((t) => t.success === false)
        ? "I'm sorry — I hit a snag with that just now. Could we try that once more?"
        : "That's all set — is there anything else I can help you with?";
    }
