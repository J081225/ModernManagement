...
// AD6 (Law 3): masking for the change notices lives beside the rest of
// the credential-security kit.
const { _reauth, GENERIC_REAUTH, maskEmail, maskPhone } = require('./credentials');
...
async function saveContactSettings(db, userId, body, attemptsMap, nowMs) {
  ...
  // AD6 (Law 3): the save reports what changed so the adapter can send
  // the notices without re-deriving anything. Old values ride along —
  // the retiring email IS a recipient.
  const changes = {
    emailChanged: emailValueChanges,
    phoneChanged: Boolean(phoneValueChanges),
    oldEmail: curEmail || null,
    newEmail: notificationEmail,
    oldPhone: curPhone || null,
    newPhone: hasAlertPhone ? alertPhone : undefined,
  };
  if (hasAlertPhone) {
    ...
    return { status: 200, body: rows[0], changes };
  }
  const { rows } = await db.query(
    `UPDATE users
       SET notification_email = $1,
...
// AD6 (Law 3): pure notice builder — recipients, masking, and message
// text with zero I/O, so the suite executes every case. Rules, ruled:
// - email change (first-set, edit, clear all count): the RETIRING
//   address is told directly, plus the anchor (users.email) when
//   distinct. Neither reachable -> one empty-recipient entry so the
//   sender logs the skip loudly.
// - phone change: one notice to the anchor, else a VERIFIED
//   notification email (fallbackEmail — the adapter resolves it
//   post-save, so an email changed in the same save can never carry
//   the phone notice), else the loud skip.
// Informational only (ruling 4): the reset flow is the response —
// never a revert link.
function buildContactChangeNotices({ changes, anchorEmail, publicBaseUrl }) {
  const notices = [];
  if (!changes) return notices;
  const anchor = String(anchorEmail || '').trim();
  const resetLine = "\n\nIf this wasn't you, reset your password now at "
    + (publicBaseUrl || '') + '/forgot-password and contact support.';

  if (changes.emailChanged) {
    const subject = 'Security notice: your notification email changed';
    let line;
    if (!changes.oldEmail) {
      line = 'A notification email was just added to your Modern Management account ('
        + maskEmail(changes.newEmail) + "). Alerts will use it once it's verified.";
    } else if (!changes.newEmail) {
      line = 'The notification email was removed from your Modern Management account. '
        + 'Alerts fall back to your account email until a new one is set and verified.';
    } else {
      line = 'The notification email on your Modern Management account changed to '
        + maskEmail(changes.newEmail) + '. This address no longer receives alerts.';
    }
    const recipients = [];
    if (changes.oldEmail) recipients.push(changes.oldEmail);
    if (anchor && anchor.toLowerCase() !== String(changes.oldEmail || '').toLowerCase()) {
      recipients.push(anchor);
    }
    if (!recipients.length) recipients.push('');
    for (const to of recipients) notices.push({ to, subject, text: line + resetLine });
  }

  if (changes.phoneChanged) {
    const subject = 'Security notice: your alert phone changed';
    let line;
    if (!changes.oldPhone) {
      line = 'An alert phone was just added to your Modern Management account ('
        + maskPhone(changes.newPhone) + "). Urgent alerts will use it once it's verified.";
    } else if (!changes.newPhone) {
      line = 'The alert phone was removed from your Modern Management account.';
    } else {
      line = 'The alert phone on your Modern Management account changed to '
        + maskPhone(changes.newPhone) + '.';
    }
    notices.push({ to: anchor || String(changes.fallbackEmail || '').trim(), subject, text: line + resetLine });
  }
...
module.exports = { validateContactSettings, saveContactSettings, buildContactChangeNotices, testAlertGate, EMAIL_RE };
