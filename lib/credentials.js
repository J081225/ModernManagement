...
// AD8 (f): security-notice sends feed the mail-outage streak monitor.
...
async function sendSecurityNotice({ sendgrid, env, logger }, to, { subject, text }) {
  ...
  if (!addr) {
    // A skip is not a send FAILURE (no email to reach) — it must not
    // feed the outage streak, or a user with no anchor would trip a
    // false alarm.
  ...
  try {
    ...
    mailHealth.recordFailure({ source: 'security-notice', reason: err.message });
    return null;
  }
}
...
// ---- AD7 (Law 4): the kill-switch -----------------------------------
// Tokens are ammunition — one password write burns every live artifact
// an attacker could hold. ONE function, called from exactly two
// triggers (in-app change keeps the current sid; the public reset
// keeps nothing), so the artifact list can never drift between them.
//
// The list, ruled in AD7's investigation:
// - sessions (all, or all-except-keepSid)
// - password_reset_tokens (plaintext at rest — the hottest item; an
//   attacker's stockpiled reset links die the moment the owner acts)
// - the AD3 pending email swap (idempotent overlap with
//   changePassword's own inline clear — a zero count there is fine)
// - AD5 contact_verifications (the public email-verify link is the
//   ONE artifact session death can't reach)
// - push_subscriptions (bearer-ish channels that would keep piping
//   message previews to an attacker's browser; the owner re-enables
//   push in one tap)
//
// Burns are INDEPENDENT: each failure logs loudly, reports null for
// its count, and never stops the others. The caller soft-fails around
// the whole call — a burn problem never blocks the password write.
async function burnCredentialArtifacts(db, userId, { keepSid, logger } = {}) {
  const log = logger || console;
  const counts = {
    sessions: null,
    reset_tokens: null,
    pending_email: null,
    contact_verifications: null,
    push_subscriptions: null,
  };
  const burn = async (key, fn) => {
    try {
      counts[key] = await fn();
    } catch (err) {
      log.error('[kill-switch] ' + key + ' burn failed:', err.message);
    }
  };
  await burn('sessions', async () => {
    if (keepSid) return endOtherSessions(db, userId, keepSid);
    const r = await db.query(
  ...
  await burn('reset_tokens', async () => {
    const r = await db.query('DELETE FROM password_reset_tokens WHERE user_id = $1', [userId]);
    return r.rowCount || 0;
  });
  await burn('pending_email', async () => {
    const r = await db.query(
      `UPDATE users
          SET pending_email = NULL,
              pending_email_token_hash = NULL,
  ...
  await burn('contact_verifications', async () => {
    const r = await db.query('DELETE FROM contact_verifications WHERE user_id = $1', [userId]);
    return r.rowCount || 0;
  });
  await burn('push_subscriptions', async () => {
    const r = await db.query('DELETE FROM push_subscriptions WHERE user_id = $1', [userId]);
    return r.rowCount || 0;
  });
  return counts;
}
