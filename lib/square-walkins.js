...
// SQW7: the "Charge in person" match. A launched charge carries our
// reference two ways: the NOTE ("Ref <id> · <business>", legacy
// "MM-TX-<id>") rides into payment.note, and the POS callback stored
// Square's transaction id on the transaction (the belt, migration 082).
// When either identifies a transaction owned by THIS workspace and the
// payment's total equals that transaction's EXACT remaining balance,
// the payment is recorded AGAINST the originating transaction — real
// provenance, the honest "automatically." Any disagreement leaves the
// tray row untouched for one-tap; nothing is guessed.
function _refFromNote(note) {
  const m = String(note || '').match(/\b(?:Ref|MM-TX-?)\s*(\d+)\b/i);
  return m ? parseInt(m[1], 10) : null;
}

async function matchAndRecordRefPayment(pool, { trayId, workspaceId, payment, logger = console }) {
  const refId = _refFromNote(payment.note);
  const posIds = [payment.order_id, payment.id].filter(Boolean);
  if (!refId && !posIds.length) return { ok: false, reason: 'no_ref' };
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const t = await client.query(
      `SELECT * FROM square_unmatched_payments WHERE id = $1 AND workspace_id = $2 AND status = 'unrecorded' FOR UPDATE`,
      [trayId, workspaceId]
    );
    const tray = t.rows[0];
    if (!tray) { await client.query('ROLLBACK'); return { ok: false, reason: 'tray_unavailable' }; }

    // Find the originating transaction: the note ref first, the stored
    // POS id as the belt. Workspace-scoped ALWAYS — a note naming some
    // other workspace's sale can never match.
    let tx = null;
    if (refId) {
      const r = await client.query(
        `SELECT * FROM transactions WHERE id = $1 AND workspace_id = $2 AND status IN ('unpaid', 'partially_paid', 'pending') FOR UPDATE`,
        [refId, workspaceId]
      );
      tx = r.rows[0] || null;
    }
    if (!tx && posIds.length) {
      const r = await client.query(
        `SELECT * FROM transactions WHERE workspace_id = $1 AND square_pos_transaction_id = ANY($2) AND status IN ('unpaid', 'partially_paid', 'pending') FOR UPDATE`,
        [workspaceId, posIds]
      );
      if (r.rows.length === 1) tx = r.rows[0];
    }
    if (!tx) { await client.query('ROLLBACK'); return { ok: false, reason: 'no_matching_transaction' }; }

    // The paid total must equal the transaction's EXACT remaining
    // balance — the guard that stops an edited note from mis-attributing
    // money (§6.2). Tips ride on top and stay part of the same payment.
    const balance = tx.total_cents - (tx.amount_paid_cents || 0);
    const paidToward = tray.amount_cents; // tip excluded from the balance test
    if (!(balance > 0) || paidToward !== balance) {
      await client.query('ROLLBACK');
      logger.error('[square-walkins] ref-match AMOUNT MISMATCH: payment ' + paidToward + 'c vs balance ' + balance + 'c on tx #' + tx.id + ' — left in the tray.');
    ...
    await client.query(
      `INSERT INTO transaction_payments
         (workspace_id, transaction_id, amount_cents, payment_type, payment_method,
          processor, processor_ref, status, notes, created_by_user_id, square_payment_id)
       VALUES ($1, $2, $3, 'payment', 'square', 'square', $4, 'completed', $5, NULL, $6)`,
      [workspaceId, tx.id, tray.total_cents,
        tray.square_order_id || tray.square_payment_id,
        'Charge in person (' + (refId ? 'Ref ' + tx.id : 'POS id') + ' match)',
        tray.square_payment_id]
    );
    await client.query(
      `UPDATE transactions
          SET amount_paid_cents = amount_paid_cents + $1,
              tip_cents = tip_cents + $2,
              status = 'paid',
              payment_method = 'card',
              payment_received_at = COALESCE(payment_received_at, $3),
              updated_at = NOW()
        WHERE id = $4`,
      [paidToward, tray.tip_cents || 0, tray.paid_at, tx.id]
    );
    await client.query(
      `UPDATE square_unmatched_payments
          SET status = 'recorded', transaction_id = $1, recorded_via = 'ref_match', updated_at = NOW()
        WHERE id = $2`,
      [tx.id, trayId]
    );
    await client.query('COMMIT');
    logger.log('[square-walkins] REF-MATCHED payment ' + payment.id + ' → transaction #' + tx.id + ' (paid in full)');
...
module.exports = { recordUnmatchedSquarePayment, recordTrayRowAsSale, matchAndRecordRefPayment, correlateMerchantSideRefund, _refFromNote, _fieldsFromPayment };
