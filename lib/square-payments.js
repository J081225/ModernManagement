...
async function createSquarePaymentLink({
  ...
  // CreatePaymentLink takes EITHER quick_pay OR order, never both, and
  // `order` (when used) is an Order object DIRECTLY. The old code sent
  // quick_pay AND a double-nested `order: { order: {...} }` — an invalid
  // payload Square rejects. We use quick_pay only (location_id lives
  // inside it). reference_id is intentionally dropped: quick_pay has no
  // reference_id field, and the webhook matches on order_id (returned
  // below), so nothing depends on it. `referenceId` stays in the
  // signature for callers but is unused until/unless we move to the full
  // order path.
  const body = {
