 ...
 * Idempotent. Recovery backstop for the past_due path: if the workspace
 * is currently 'past_due', promote it back to 'active'. For any other
 * current status ('active', 'incomplete', 'canceled', etc.) it's a
 * deliberate no-op — a successful payment doesn't un-cancel a
 * subscription or finalize an incomplete one. subscription.updated
 * remains the source of truth for those transitions; this handler only
 * exists so a past_due workspace recovers cleanly when Stripe fires
 * payment_succeeded without an accompanying subscription.updated.
 * Mirrors processInvoicePaymentFailedEvent in shape and error handling.
 */
async function processInvoicePaymentSucceededEvent(event, pool, ctx = {}) {
  const eventId = event.id;
  const invoice = event.data && event.data.object;
  if (!invoice || !invoice.subscription) {
    return { skipped: true, reason: 'missing_subscription_on_invoice' };
  ...
  try {
    ...
    // Only promote past_due → active. Other statuses are left alone —
    // a payment succeeding does not un-cancel, un-incomplete, or
    // otherwise mutate a workspace that's in a non-past_due state.
    if (workspace.subscription_status !== 'past_due') {
      ...
      return {
        skipped: true,
        reason: 'workspace_not_past_due',
        current_status: workspace.subscription_status,
      };
    }

    await client.query(
      `UPDATE workspaces SET subscription_status = 'active' WHERE id = $1`,
      [workspace.id]
    );
    ...
    await logChange(pool, workspace.id, workspace.owner_user_id, 'invoice_payment_succeeded', {
      old_status: workspace.subscription_status,
      new_status: 'active',
      stripe_invoice_id: invoice.id,
      stripe_subscription_id: invoice.subscription,
    ...
    return { processed: true, workspace_id: workspace.id, recovered_from: 'past_due' };
  } catch (err) {
    await client.query('ROLLBACK').catch(() => {});
    console.error('[subscription-lifecycle] invoice.payment_succeeded processing failed:', err);
    throw err;
  } finally {
    client.release();
  }
...
module.exports = {
  ...
  processInvoicePaymentSucceededEvent,
