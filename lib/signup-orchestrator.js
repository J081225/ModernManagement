...
function welcomeEmailSubject(vertical) {
  if (vertical === 'professional-services') {
    return 'Welcome to Modern Management — your AI Manager is ready';
  }
...
function welcomeEmailHtmlPS({ businessName, username, twilioPhone, plan, billing, baseUrl }) {
  ...
  return [
    ...
    '<div style="background:white;border-radius:12px;padding:32px;box-shadow:0 2px 12px rgba(0,0,0,0.04);">',
    '<h1 style="margin:0 0 8px;font-size:1.4em;color:#2d3748;">Welcome to Modern Management</h1>',
    '<p style="color:#64748b;margin:0 0 20px;">Hi ' + escapeHtml(businessName) + ' &mdash; your AI Manager is ready.</p>',
    // Prominent phone callout
    '<div style="background:linear-gradient(135deg,#fff7ed,#ffedd5);border:1px solid #fed7aa;border-radius:10px;padding:18px;margin:22px 0;">',
    '<div style="font-size:0.78em;font-weight:700;color:#9a3412;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:8px;">Your business phone number</div>',
    '<div style="font-size:1.6em;font-weight:800;color:#c2410c;letter-spacing:-0.3px;">' + escapeHtml(pb.value) + '</div>',
    ...
    '<li>Connect your phone number to your AI: open My Business and turn on auto-respond under &ldquo;How your Manager works&rdquo; when you\'re ready to let the AI reply on its own.</li>',
    '</ul>',
...
function welcomeEmailTextPS({ businessName, username, twilioPhone, plan, billing, baseUrl }) {
  ...
  return [
    'Welcome to Modern Management',
    '',
    'Hi ' + businessName + ' — your AI Manager is ready.',
    '',
    'YOUR BUSINESS PHONE NUMBER:',
    '  ' + pb.value,
    '',
    ...
    "* Connect your phone number to your AI: open My Business and turn on auto-respond under \"How your Manager works\" when you're ready to let the AI reply on its own.",
    '',
...
async function processCheckoutCompletedEvent(event, pool, stripeClient) {
  ...
  try {
    ...
    // VE2: the username IS the address. draft.username was validated
    // lowercase/unique/reserved-gated (VE1) at checkout; the old
    // user-<token> inbound-subdomain format was NXDOMAIN (never
    // deliverable) and must never be minted again. Root domain only —
    // pinned by VA4 (no inbound-subdomain literal may reappear here).
    const inboundEmailAlias = String(draft.username).trim().toLowerCase() + '@modernmanagementapp.com';
