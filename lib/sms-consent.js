...
// CONSENT-EVENTS (083): every state write ALSO appends an append-only
// evidence row — the verbatim keyword, direction, and MessageSid — in
// the SAME statement via a data-modifying CTE (same-transaction by
// construction: both land or neither does). sms_opt_outs stays the
// fast send-gate; sms_consent_events is liability history — INSERT-
// only, never purged, no UPDATE/DELETE path exists in code.
async function recordOptOut(db, workspaceId, phone, keyword, messageSid) {
  await db.query(
    `WITH state AS (
     ...
     INSERT INTO sms_consent_events (workspace_id, phone, keyword, direction, message_sid)
     VALUES ($1, $2, $3, 'opt_out', $4)`,
    [workspaceId, phone, (keyword || 'STOP').slice(0, 40), messageSid || null]
  );
}

async function recordOptIn(db, workspaceId, phone, keyword, messageSid) {
  await db.query(
    `WITH state AS (
     ...
     INSERT INTO sms_consent_events (workspace_id, phone, keyword, direction, message_sid)
     VALUES ($1, $2, $3, 'opt_in', $4)`,
    [workspaceId, phone, (keyword || 'START').slice(0, 40), messageSid || null]
  );
}

// CONSENT-EVENTS: HELP has no state to upsert — evidence row only.
async function recordHelpEvent(db, workspaceId, phone, keyword, messageSid) {
  await db.query(
    `INSERT INTO sms_consent_events (workspace_id, phone, keyword, direction, message_sid)
     VALUES ($1, $2, $3, 'help', $4)`,
    [workspaceId, phone, (keyword || 'HELP').slice(0, 40), messageSid || null]
  );
}
...
module.exports = {
  classifyOptKeyword, recordOptOut, recordOptIn, recordHelpEvent, isOptedOut,
