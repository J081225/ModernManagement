  ...
  try {
    ...
    for (const s of suggestions.slice(0, MAX_SUGGESTIONS_PER_CONVERSATION)) {
      ...
      if (s && s.expense && typeof s.expense === 'object') {
        const ex = s.expense;
        const amount = (typeof ex.amount_cents === 'number' && Number.isInteger(ex.amount_cents) && ex.amount_cents > 0) ? ex.amount_cents : null;
        const CATS = ['Supplies', 'Payroll', 'Rent', 'Utilities', 'Marketing', 'Fees', 'Other'];
        const category = CATS.includes(ex.category) ? ex.category : null;
        const vendor = (typeof ex.vendor === 'string' && ex.vendor.trim()) ? ex.vendor.trim().slice(0, 100) : null;
        expenseMarker = JSON.stringify({ amount_cents: amount, category, vendor });
        if (!title.toLowerCase().startsWith('post ')) {
          title = (amount != null && category)
            ? ('Post $' + (amount / 100).toFixed(2) + ' to ' + category + '?').slice(0, TITLE_CAP)
