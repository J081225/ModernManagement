...
const STRINGS = {
  ...
  // GREET-BY-NAME: the known-caller variant — FIRST name only, same
  // MR1 automated-manager disclosure in every language (ar inherits
  // through the same ARABIC_VOICE_ENABLED flip as the generic).
  voice_greeting_known: {
    en: ({ businessName, firstName }) => `Hi ${firstName} — thanks for calling ${businessName}, this is their automated manager. How can I help you today?`,
    es: ({ businessName, firstName }) => `Hola ${firstName} — gracias por llamar a ${businessName}, le habla su gerente automatizado. ¿En qué puedo ayudarle hoy?`,
    ar: ({ businessName, firstName }) => `مرحباً ${firstName} — شكراً لاتصالكم بـ${businessName}، معكم المدير الآلي. كيف أقدر أساعدكم اليوم؟`,
  },
