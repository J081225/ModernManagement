...
// Last-resort fallback: search for ANY available US local number that has
// SMS + Voice enabled, without filtering by area code. Used by the signup
// orchestrator when the customer's requested area codes are dry — better
// to give the customer some working US number than to fail the signup
// (their Stripe charge has already succeeded at that point).
//
// Returns the SAME shape as searchAvailableNumbers so callers can consume
// results identically. Same Twilio errors of note.
async function searchAnyAvailableNumber(limit = 5) {
  if (!Number.isFinite(limit) || limit < 1 || limit > 30) {
    throw new Error('limit must be an integer 1-30');
  }
  try {
    ...
    console.error('[twilio-provisioning] searchAnyAvailableNumber failed: code=' + (err.code || 'unknown') + ', message=' + err.message);
    throw err;
  }
}
...
// SP5a: the voice path is VERTICAL-DEPENDENT and this function used to
// hardcode the PM one for every number. Consequences it caused:
//   - every PS number provisioned by the SP4a worker got the PM
//     voicemail path from birth, so a salon's caller heard "Thank you
//     for calling Modern Management. Please leave your message after
//     the beep" and the AI receptionist was silently bypassed;
//   - any repair re-run pointed at a PS number would clobber its
//     working relay path into the same wrong greeting.
// PS numbers answer with the ConversationRelay entry TwiML; PM numbers
// answer with the voicemail TwiML. SMS is identical for both.
const VOICE_PATH_BY_VERTICAL = {
  'professional-services': '/api/voice/relay-incoming',
  'property-management':   '/api/voice/incoming',
};
// Unknown/missing vertical falls back to the PM voicemail path: it is
// the safe default (a caller reaches a recording that a human reads)
// rather than an AI socket for a workspace that may not want one.
const DEFAULT_VOICE_PATH = '/api/voice/incoming';

function voicePathForVertical(vertical) {
  return VOICE_PATH_BY_VERTICAL[vertical] || DEFAULT_VOICE_PATH;
}

// configureNumberWebhooks(phoneSid, webhookBaseUrl, options)
//   options.vertical — 'professional-services' | 'property-management'
// Back-compatible: called without options it behaves exactly as before
// (PM paths), so the legacy manual harness keeps working unchanged.
async function configureNumberWebhooks(phoneSid, webhookBaseUrl, options = {}) {
...
// PROV-ATTACH: put a freshly provisioned number on the A2P Messaging
// Service so it can send SMS under the VERIFIED campaign. The service
// SID comes from env (TWILIO_MESSAGING_SERVICE_SID) — never hardcoded.
// Throws when the env var is missing or the API call fails; the CALLER
// decides that failure is loud-but-not-fatal (the worker never rolls a
// signup back over a texting gap).
async function attachToMessagingService(phoneSid, env = process.env) {
  const serviceSid = env.TWILIO_MESSAGING_SERVICE_SID;
  if (!serviceSid) {
    throw new Error('TWILIO_MESSAGING_SERVICE_SID not set — number NOT attached to the A2P messaging service');
  }
  if (!/^PN[0-9a-f]{32}$/.test(String(phoneSid))) {
    throw new Error('attachToMessagingService: phoneSid must be a PN sid');
  }
  const result = await getClient().messaging.v1.services(serviceSid).phoneNumbers.create({ phoneNumberSid: phoneSid });
  return { service_sid: serviceSid, phone_sid: (result && result.phoneNumberSid) || phoneSid };
}
...
module.exports = {
  ...
  // SP5a: exported so the repair run (SP5b) and the suite can compute
  // the correct voice path without duplicating the mapping.
  voicePathForVertical,
  VOICE_PATH_BY_VERTICAL,
};
