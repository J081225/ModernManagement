...
function workspaceReadiness(workspace) {
  const w = workspace || {};
  const cards = w.connect_status || 'not_started';
  const phone = phoneAxis(w);
  const cards_ready = cards === 'ready';
...
module.exports = { TWILIO_STATUSES, phoneAxis, workspaceReadiness, assertPhoneStatusLegal };
