...
async function appendCallTurn(db, messageId, speaker, text) {
  ...
  // SPEECH-GRADE part 3 (telemetry, additive): every turn carries a
  // UTC wall-clock stamp so dead-air gaps are measurable from the
  // transcript instead of unknowable.
  const stamp = new Date().toISOString().slice(11, 19) + 'Z';
  const line = `[${stamp}] ${speaker}: ${text}`;
  await db.query(
    `UPDATE messages
