...
registry.register({
  ...
  schema: {
    ...
    properties: {
      ...
      open_question: { type: 'string', description: 'Optional (R2 MINIMAL-ASK). Any unresolved booking detail (service variant, add-on, preference) — files an owner follow-up task instead of asking the customer more questions.' },
  ...
  async execute(input, ctx) {
    ...
    // BH0 (rung 1): closed weekdays are unbookable AT THE TOOL — no
    // prompt phrasing can write onto a closed day. Weekday computed in
    // the WORKSPACE timezone (a late-evening UTC skew must not move
    // the day).
    const BH_WEEKDAYS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const bhDayName = startsAtDate.toLocaleString('en-US', { timeZone: wsTz(ctx.workspace), weekday: 'long' });
    if (new Set(ctx.workspace.closed_weekdays || []).has(BH_WEEKDAYS.indexOf(bhDayName))) {
      return {
        success: false,
        reason: 'closed_that_day',
        message: `The business is closed on ${bhDayName}s — that day can't be booked. Offer the customer the nearest open day.`,
      };
    }
    ...
    if (ctx.origin && ctx.origin.appointment_thread_id) {
      try {
        // SPEECH-GRADE part 2: on a LIVE CALL the thread stays open for
        // the rest of the call — a caller who just booked must never
        // meet a stranger ("actually, make it 6" keeps full context).
        // The relay marks it complete at call TEARDOWN, not here. Text
        // channels keep the original lifecycle.
        const liveCall = ctx.origin.channel === 'ai_inbound' && ctx.origin.channel_detail === 'voice';
        if (liveCall && initialStatus === 'confirmed') {
          await ctx.db.query(
            `UPDATE appointment_threads
                SET appointment_id = $1, updated_at = NOW()
              WHERE id = $2`,
            [appointment.id, ctx.origin.appointment_thread_id]
          );
        } else {
          await ctx.db.query(
            `UPDATE appointment_threads
    ...
    // R2 MINIMAL-ASK: an unresolved detail becomes an owner follow-up
    // task (suggested), naming the customer, the gap, and the thread —
    // instead of another question to the customer. Best-effort.
      ...
      try {
        await ctx.db.query(
          `INSERT INTO tasks (user_id, title, category, "dueDate", notes, suggested, "aiReason")
           VALUES ($1, $2, 'follow_up', $3, $4, true, $5)`,
          [ctx.workspace.owner_user_id,
            `Follow up with ${customer_name} about their booking`,
            new Date().toISOString().slice(0, 10),
            `Open question from the ${title} booking (appointment #${appointment.id}`
            + (ctx.origin && ctx.origin.appointment_thread_id ? `, thread #${ctx.origin.appointment_thread_id}` : '')
            + `): ${String(input.open_question).slice(0, 400)}`,
            'Minimal-ask booking: one clarifying question max — the rest comes to you.']
        );
      } catch (taskErr) {
        ctx.logger.error('[book_appointment] open_question task failed (booking stands):', taskErr.message);
      }
    }
    ...
    const tzName = wsTz(ctx.workspace);
    const niceTime = startsAtDate.toLocaleString('en-US',
      { timeZone: tzName, weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true });
    const timePart = startsAtDate.toLocaleString('en-US',
      { timeZone: tzName, hour: 'numeric', minute: '2-digit', hour12: true });
    const datePart = startsAtDate.toLocaleString('en-US',
      { timeZone: tzName, weekday: 'long', month: 'long', day: 'numeric' });
    // R5a: when Owner review is on and the booking came from a CUSTOMER
    // conversation, the tool hands the model the ruled caller-facing
    // line — never process words ("pending", "queue", "waiting on
    // approval"). Owner-initiated bookings keep the owner-facing shape.
    const isCustomerBooking = !!(ctx.origin && ctx.origin.channel === 'ai_inbound');
    const userMessage = (initialStatus === 'confirmed'
      ...
      : isCustomerBooking
        ? `I have you booked for ${title} at ${timePart} on ${datePart} — it just needs the owner's approval, and you'll get a text confirmation as soon as it's confirmed.`
