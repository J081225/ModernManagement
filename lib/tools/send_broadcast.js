...
const { isOptedOut, looksPromotional } = require('../sms-consent');
...
registry.register({
  ...
  async execute(input, ctx) {
    ...
    // Ruling 7 (A2P): broadcasts are SERVICE notices only (closures,
    // delays, account/appointment updates). Promotional content is not
    // allowed on a transactional campaign — it needs a separate Marketing
    // campaign with its own consent. Refuse it here.
    // LP2a: the demo workspace never sends SMS (A2P pending) — refuse the
    // whole broadcast up front rather than skipping per-recipient.
    if (channel === 'sms' && ctx.workspace && ctx.workspace.is_demo) {
      return { success: false, message: 'This is the demo workspace — it cannot send SMS broadcasts.' };
    }
    if (looksPromotional(body) || (subject && looksPromotional(subject))) {
      return {
        success: false,
        message: 'That reads as promotional. Broadcasts are for service notices only (e.g. a closure, delay, or account update). Marketing or promotions need a separate messaging campaign — I can\'t send this here.',
      };
    }
    ...
    for (const r of sendable) {
      // A2P/TCPA: never broadcast to an opted-out number (per workspace).
      if (channel === 'sms' && await isOptedOut(ctx.db, ctx.workspace.id, r.phone)) {
        optedOutSkipped++;
        continue;
      }
      try {
    ...
    if (optedOutSkipped > 0) summary += ` (${optedOutSkipped} skipped — opted out)`;
