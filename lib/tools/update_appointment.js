...
registry.register({
  ...
  async execute(input, ctx) {
    ...
    // FD2: inbound customers may only touch their own appointment, and
    // even then may only RESCHEDULE. Price, service, duration, and
    // notes stay owner-only — negotiation happens with the owner, not
    // the database.
    let customerStrippedNote = '';
    if (ctx.origin && ctx.origin.channel === 'ai_inbound') {
      const owns = await callerOwnsAppointment(ctx, current);
      if (!owns) {
        await escalateScopeMismatch(ctx, current, 'change');
        return { success: false, message: SCOPE_REFUSAL };
      }
      const blocked = ['title', 'notes_internal', 'notes_customer', 'quoted_price_cents', 'duration_minutes']
        .filter((k) => input[k] !== undefined);
      if (blocked.length && input.starts_at === undefined) {
        return { success: false, message: 'I can only change the time of your appointment. For price or service changes, the owner will confirm with you directly.' };
      }
      for (const k of blocked) delete input[k];
      if (blocked.length) customerStrippedNote = ' (Only the time was changed — price or service updates need the owner.)';
    }
    ...
    return { success: true, data: { appointment_id }, message: `Updated appointment #${appointment_id}.${customerStrippedNote}` };
  },
});
