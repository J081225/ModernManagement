...
registry.register({
  ...
  async execute(input, ctx) {
    ...
    // B4 (AI-scope): the same premium-model cost guard as POST
    // /api/reports — one cap, both creation sites.
    const { reportCapExceeded, REPORT_CAP_MESSAGE } = require('../report-cap');
    if (await reportCapExceeded(ctx.db, ctx.workspace.id)) {
      return { success: false, message: REPORT_CAP_MESSAGE };
    }
