...
registry.register({
  ...
  async execute(input, ctx) {
    ...
    return {
      ...
      message: `Refunded $${(amount_cents / 100).toFixed(2)} on transaction #${parent.id} (${newStatus === 'refunded' ? 'fully refunded' : 'partial'}).`,
      summary: `Refund $${(amount_cents / 100).toFixed(2)} on transaction #${parent.id}`,
    };
  },
});
