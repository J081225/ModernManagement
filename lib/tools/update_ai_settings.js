...
registry.register({
  ...
  async execute(input, ctx) {
    if (ctx.origin && ctx.origin.channel === 'ai_inbound') {
      return { success: false, message: 'Only the business owner can change Manager settings.' };
    }
    const setClauses = [];
    const params = [];
    let i = 1;
    ...
    return { success: true, data: { changed: changed.split(', ') }, message: `Manager settings updated: ${changed}.`, summary: `Change Manager settings (${changed})` };
  },
});
