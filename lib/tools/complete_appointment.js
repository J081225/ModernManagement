...
registry.register({
  ...
  async execute(input, ctx) {
    // FD2 belt-and-suspenders: completing (and its payment writes) is
    // owner-only; a customer conversation can never reach it even if
    // the engine allowlist drifts.
    if (ctx.origin && ctx.origin.channel === 'ai_inbound') {
      return { success: false, message: 'Only the business owner can mark an appointment completed or record payment.' };
    }
    ...
    try {
      ...
      if (apt.rows.length > 0) {
        ...
        try {
          ...
          // FD3-CP6: an appointment that took a booking-time deposit
          // already HAS its transaction (deposit_transaction_id).
          // Creating a second one here would double-book the revenue,
          // so completion updates THAT transaction's line items and
          // totals instead, and the deposit already in its ledger
          // counts toward the balance automatically.
          if (a.deposit_transaction_id) {
            await client.query(
              `UPDATE transactions
                  SET line_items = $1::jsonb, subtotal_cents = $2, tip_cents = $3,
                      total_cents = $4, payment_method = COALESCE($5, payment_method),
                      updated_at = NOW()
                WHERE id = $6 AND workspace_id = $7`,
              [JSON.stringify(lineItems), subtotal, tip, total, pm,
                a.deposit_transaction_id, ctx.workspace.id]
            );
            draft_transaction_id = a.deposit_transaction_id;
          } else {
          ...
          // and status. For a deposit-carrying transaction the new total
          // may also change the rollup verdict, so recompute runs even
          // when no new money arrived.
          if (a.deposit_transaction_id && paid <= 0) {
