...
registry.register({
  ...
  async execute(input, ctx) {
    ...
    const dayStart = new Date(`${target_date}T${String(DEFAULT_DAY_START_HOUR).padStart(2, '0')}:00:00`);
    const dayEnd = new Date(`${target_date}T${String(DEFAULT_DAY_END_HOUR).padStart(2, '0')}:00:00`);
    ...
    const niceList = picked
      .map((s) => new Date(s.starts_at).toLocaleString('en-US', { hour: 'numeric', minute: '2-digit' }))
      .join(', ');
    return {
      success: true,
      data: { slots: picked },
