...
const { customerSmsFrom, cardsReady } = require('./workspace-readiness');
...
async function createPaymentRequest({
  ...
  // SQ3: gate on the derived helper, not a raw column. This is the
  // STRIPE request path, so it also needs the Stripe account; for an
  // all-Stripe workspace cardsReady() === (connect_status === 'ready'),
  // so behavior is identical. A Square-active workspace fails here (no
  // stripe account) and is routed to the Square path in SQ4.
  if (!cardsReady(workspace) || !workspace.stripe_connect_account_id) {
  ...
  if (customerPhone && twilio && fromNum) {
    try {
      // ST5a: the link SMS comes from the declared-variants module in
      // the workspace's customer language; the amount formats through
      // lib/money (one fewer inline site — TX19 re-pins downward).
      const body = customerString(workspace.customer_language || 'en', 'payment_link_sms', {
