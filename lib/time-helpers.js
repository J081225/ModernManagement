...
// FD3-CP7: today's date in the WORKSPACE's timezone (en-CA renders
// YYYY-MM-DD). The two residual UTC-date sites (the task suggester and
// the daily nudge) switch to this — after ~7-8 PM Eastern a bare
// toISOString() date is tomorrow's.
function wsToday(workspace) {
  try {
    return new Date().toLocaleDateString('en-CA', { timeZone: wsTz(workspace) });
  } catch (err) {
    return new Date().toISOString().slice(0, 10);
  }
}

module.exports = { DEFAULT_TZ, wsTz, toZonedISO, promptTimeAnchor, wsToday };
