...
const { squareBase, squareSessionParam } = require('./square-env');
...
function authorizeUrl({ state, redirectUri }) {
  ...
  // `session` is environment-aware (lib/square-env). Omitted in both
  // envs today → Square's default session=true. It is NEVER 'false' in
  // sandbox: that forces a login sandbox test sellers can't perform and
  // lands on a blank page (the live bug this replaced).
  const session = squareSessionParam();
